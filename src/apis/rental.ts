...
// 물품 리스트 조회
export const getItemsList = async (): Promise<
  ApiResponse<SetStateAction<Items[]>>
> => {
  try {
    const response =
      await axiosInstance.get<ApiResponse<SetStateAction<Items[]>>>(
        "/api/items",
      );
...
export const getItemDetail = async (
  ...
  try {
    ...
    console.error(
      `아이템 세부 정보 조회 중 오류 발생 (itemId: ${itemId}):`,
      error,
    );
